...
def _opentdb_schema(
    categories: list[dict[str, Any]], defaults: dict[str, Any] | None = None
) -> vol.Schema:
    values = defaults or {}
    category_options = [
            ...
            if user_input:
                try:
                    ...
                    return self.async_create_entry(
                        title="", data={**user_input, CONF_SOURCE: SOURCE_OPENTDB}
                    )
            return self.async_show_form(
                step_id="opentdb",
                data_schema=_opentdb_schema(categories, {**defaults, **(user_input or {})}),
                errors=errors,
            )
        ...
        async def async_step_file(self, user_input: dict[str, Any] | None = None) -> Any:
            ...
            if user_input:
                try:
                    ...
                    return self.async_create_entry(
                        title="", data={**user_input, CONF_SOURCE: SOURCE_FILE}
                    )
            return self.async_show_form(
                step_id="file",
