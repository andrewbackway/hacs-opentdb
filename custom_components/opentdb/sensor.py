...
SENSORS: tuple[QuizSensorDescription, ...] = (
    ...
    QuizSensorDescription(
        key="quiz_statistics",
        translation_key="quiz_statistics",
        icon="mdi:chart-box-outline",
        value_fn=lambda data: (data.get("quiz_stats") or {}).get("questions", 0),
        attributes_fn=lambda data: data.get("quiz_stats") or {},
    ),
    QuizSensorDescription(
        key="leaderboard",
        translation_key="leaderboard",
        icon="mdi:podium",
        value_fn=lambda data: len(data.get("leaderboard") or []),
        attributes_fn=lambda data: {"leaderboard": data.get("leaderboard") or []},
    ),
)
...
class QuizSensor(CoordinatorEntity[QuizDataUpdateCoordinator], SensorEntity):
    ...
    def __init__(
        self,
        coordinator: QuizDataUpdateCoordinator,
        entry: ConfigEntry,
        description: QuizSensorDescription,
    ) -> None:
        ...
        self._attr_device_info = DeviceInfo(
            ...
            name=entry.title,
            manufacturer="Open Trivia Database",
            model="Quiz",
        )
    ...
    def extra_state_attributes(self) -> dict[str, Any]:
        return {
            key: value
            for key, value in self.entity_description.attributes_fn(
